...
let string_of_args args_pls =
  ...
  in
  if List.length args_pls > 0 then String.sub args 2 (String.length args - 2) else args
...
let string_of_fields fields fields_pls =
  ...
  let args =
    List.fold_left
      ...
      "" fields_pls
  in
  if List.length fields_pls > 0 then String.sub args 2 (String.length args - 2) else args
...
let emit_instr fmt prog label instr =
  ...
  | Iif (cond_pl, if_label, else_label) ->
      emit_label ();
